...
package com.huidian.hadoop.hbase.dataExport;
...
public class ExportSnapshot extends Configured implements Tool {
    ...
    private void runCopyJob(final Path inputRoot, final Path outputRoot,
                            ...
                            final int mappers, final int bandwidthMB, String snapshotOldName, Path snapShotOldDir)
    ...
    public int run(String[] args) throws IOException {
        ...
        String snapshotOldName = null;//【新增】对比快照参数
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if (cmd.equals("-snapshot")) {
                snapshotName = args[++i];
            } else if (cmd.equals("-snapshot-old")) {//【新增】对比快照参数
        ...
        Path snapShotOldDir = null;//【新增】获取旧快照目录
        if (snapshotOldName != null) {
        ...
        try {
            runCopyJob(inputRoot, outputRoot, snapshotName, snapshotDir, verifyChecksum,
                    filesUser, filesGroup, filesMode, mappers, bandwidthMB, snapshotOldName, snapShotOldDir);//【修改】添加对比快照名称及路径参数
